...
var commands = map[string]string{
	"apply": `Run ` + "`clanopy apply`" + ` to apply this worktree session's changes to the main workspace. The command auto-detects the current session name from the working directory.

If it fails because another session is already applied, tell the user to run ` + "`/unapply`" + ` first.
`,
	"unapply": `Run ` + "`clanopy unapply`" + ` to revert previously applied worktree session changes from the main workspace.

If no session is currently applied, let the user know.
`,
}

// Install writes the clanopy slash commands to ~/.claude/commands/clanopy/
...
func Install() (string, error) {
	...
	dir := filepath.Join(commandsDir, "clanopy")
...
func Ensure() {
	...
	// (e.g. binary was upgraded but `clanopy init` wasn't re-run).
