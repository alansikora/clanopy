...
const MarkerFileName = ".clanopy-applied"
...
func Apply(workspacePath, sessionName string, applyMarker ...bool) error {
	if applied, ok := HasAppliedSession(workspacePath); ok {
		return fmt.Errorf("session %q is already applied; run 'clanopy unapply' first", applied)
	}
	...
	if strings.TrimSpace(status) != "" {
		if _, err := runGit(workspacePath, "stash", "push", "-m", "clanopy-auto-stash"); err != nil {
