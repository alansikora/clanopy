...
func ConfigDir() string {
	if dir := os.Getenv("CLANOPY_CONFIG_DIR"); dir != "" {
		return dir
	}
	// Support deprecated LLMUX_CONFIG_DIR for migration
	if dir := os.Getenv("LLMUX_CONFIG_DIR"); dir != "" {
		return dir
	}
	home, _ := os.UserHomeDir()
	clanopyDir := filepath.Join(home, ".config", "clanopy")
	// Fall back to legacy ~/.config/llmux/ if clanopy dir doesn't exist yet
	if _, err := os.Stat(clanopyDir); os.IsNotExist(err) {
		legacyDir := filepath.Join(home, ".config", "llmux")
